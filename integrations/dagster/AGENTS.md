...
adapter over [`rocky-sdk`](../../sdk/python/AGENTS.md)'s `RockyClient` that maps
...
2. **Resource** (`resource.py`) — `RockyResource`, a `ConfigurableResource` that
   ...
   resolvers, configurable `timeout_seconds`, the strict-doctor startup gate, the governance pre-flight (raises
...
uv run ruff format --check src/ tests/        # Check only — its own CI gate (pytest green ≠ format green)
...
- **Selection / declared-check filtering**: Rocky always runs at source granularity, so a single `rocky run` may emit results for tables outside the requested subset, and may emit check kinds (e.g. `null_rate`) the component never declared. Both are filtered out of the asset-check stream — Dagster raises `DagsterInvariantViolationError` on a result outside the declared `check_specs`, which fails the step. **Both execution paths filter on the same `(asset_key, check_name)` set**: `_emit_results` for streaming, `_run_filters_pipes` for pipes (which used to filter by asset key only and so failed the step, #1673). An undeclared result is not dropped silently: `_undeclared_check_observation` logs a warning and carries the verdict as an `AssetObservation`.
- **Group checks**: `cross_source_overlap` is evaluated once per `(source_type, table)` sibling group and reported on the sibling the engine copied first — not knowable at spec-build time. Its specs carry `GROUP_CHECK_METADATA_KEY`, and `_emit_placeholder_checks` yields **nothing** for the siblings without this run's verdict — one INFO line naming the sibling that carried it, and no result. Not a passing placeholder (a green verdict for a check that never ran) and not a failing one (permanent warnings on every run). Dagster records a declared check with no evaluation as `PLANNED`, which `AssetCheckExecutionRecord.resolve_status` maps to `SKIPPED` on a finished, non-failed run (#1669).
