...
| `contain_failures` | bool | `false` | `false` stops the run at the first model that fails **while it runs**, and leaves every model it has not yet built unbuilt. A model that fails to **compile** does not stop the run: it is excluded, and the other models still build. `true` withholds the failed model and everything downstream of it, and lets unrelated subtrees finish. It reports `PartialFailure` with a manifest naming what failed and its blast radius. |
