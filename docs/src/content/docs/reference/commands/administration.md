...
Inspect a recorded run from the state store. Surfaces the per-model SQL hashes, row counts, bytes, and timings captured by `RunRecord` at execution time: the concrete artefact behind the reproducibility claim. Inspection is the default; `--check` runs a read-only replayability audit and `--execute` re-runs the recorded recipe on a local DuckDB engine.
...
| `--check` | `bool` | `false` | Run a read-only replayability audit instead of the inspection view: classifies whether each model's recording alone is sufficient to re-execute it. Nothing is executed. |
| `--execute` | `bool` | `false` | Re-execute the recorded recipe on a local in-memory DuckDB engine (reconstructed from the recording, never the working tree) and re-derive the output hash. Single, self-contained models only. |
