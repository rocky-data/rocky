...
`rocky plan` then `rocky apply` is the auditable two-step for production and PR gating: the persisted plan is reviewable before anything writes. For local iteration and automation, `rocky run` does the same work in one step:
...
- [Data quality checks](/features/data-quality-checks/): assertions that run inline with the load.
