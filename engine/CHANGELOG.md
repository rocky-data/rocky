...
- **`unique_key` columns for a `merge` strategy are now validated against the model's output schema at compile time (E036).** A `unique_key` naming a column the model does not produce previously compiled clean and only failed later, when the warehouse rejected the generated `MERGE ... ON` clause. E036 is the merge-key sibling of the E020 `time_column` existence check: one diagnostic is emitted per missing key, so a composite `unique_key` reports every typo in a single compile rather than one per fix-recompile round. The check is skipped when the model's output columns cannot be fully enumerated (a `SELECT *` over an upstream with no known schema), mirroring the existing `type_known` guard, and covers `strategy = "merge"` — the only `unique_key` the compiler can see. (#1200)
