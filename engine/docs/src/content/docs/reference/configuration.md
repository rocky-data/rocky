...
Rocky reads a single `rocky.toml` file for all configuration. For single-adapter projects, an **unnamed shorthand** keeps config minimal. Multi-adapter projects use **named adapters** (`[adapter.NAME]`) and **named pipelines** (`[pipeline.NAME]`).

## Minimal config (DuckDB)
...
That's it — 6 lines for a working project. `type = "replication"` is the default pipeline type, `adapter` refs default to the unnamed adapter, and `[state] backend = "local"` is the default.

## Named adapters (multi-adapter)

When you have multiple adapters (e.g., Databricks + Fivetran), use named adapter blocks and reference them explicitly:
...
[adapter.snow]
type = "snowflake"
# Snowflake-specific fields are documented in the Snowflake adapter docs.
```
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `adapter` | string | No | Name of the warehouse adapter. Defaults to `"default"` (the unnamed `[adapter]` form). Must match a `[adapter.NAME]` key in multi-adapter configs. |
...
Given `client=acme`, `regions=us_west`, `connector=shopify`:

| Template | Result |
|----------|--------|
| `{client}_warehouse` | `acme_warehouse` |
| `staging__{regions}__{connector}` | `staging__us_west__shopify` |
...
A credential-free DuckDB pipeline (good for examples and tests):
...
`rocky validate` checks your config for errors and emits actionable lint suggestions:

```bash
rocky validate                    # text output (default with -o table)
rocky validate --output json      # machine-readable output
```
...
| Code | Message | Action |
|------|---------|--------|
| L001 | `name` matches filename stem | Remove `name` — it's inferred |
| L002 | `target.table` matches `name` | Remove `target.table` — it defaults to `name` |
| L003 | Multiple models share `target.catalog`/`schema` | Add `models/_defaults.toml` |
| L004 | `[state] backend='local'` is the default | Remove the `[state]` block |
| L005 | `type='replication'` is the default | Remove `type` from pipeline |
| L006 | `auto_create_catalogs=false` is the default | Remove the governance line |
| L007 | Single adapter repeated N times | Use unnamed `[adapter]` shorthand |
| L008 | Auto-discovered contract (informational) | No action needed |

Lint messages have severity `"lint"` and never cause validation to fail.
