...
An `incremental` model with a `unique_key` is imported as `merge` from the same compiled SQL. `merge` creates its table from that SQL on the first run. If the SQL kept an `is_incremental()` filter, the first run loads only the recent rows. If the filter reads the model's own table, the run fails instead ([#2059](https://github.com/rocky-data/rocky/issues/2059)). The importer does not refuse this case. Remove the filter from the model's SQL before the first run.
:::
