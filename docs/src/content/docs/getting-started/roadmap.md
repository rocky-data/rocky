...
- **Branches and run records.** Named branches as isolated schemas, and a content-addressed record of each run that you inspect and verify against the ledger with `rocky replay`. Local re-execution from the pinned record has shipped for single, self-contained models: `rocky replay --execute --verify` reconstructs the recipe from the recording, re-runs it on an in-memory DuckDB engine, and compares the re-derived hash against the recorded one. Re-execution with pinned inputs for multi-model and warehouse-backed runs is on the 2026 roadmap.
