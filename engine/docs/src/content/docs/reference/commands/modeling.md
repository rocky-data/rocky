...
When invoked with `--declarative`, `rocky test` runs `[[tests]]` declared in model sidecar TOML files against the configured warehouse adapter (not DuckDB). This validates data quality assertions on production tables.

```bash
rocky test --declarative [flags]
```

#### Additional flags (declarative mode)

| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--declarative` | `bool` | `false` | Run declarative tests from model sidecars instead of local DuckDB tests. |
| `--config <PATH>` | `PathBuf` | `rocky.toml` | Path to `rocky.toml` (required — determines which warehouse adapter to use). |
| `--pipeline <NAME>` | `string` | | Pipeline name (required if multiple pipelines are configured). |
...
```
Declarative tests: 6 total

  ✓ fct_orders.order_id [not_null]
  ✓ fct_orders.order_id [unique]
  ⚠ fct_orders.status [accepted_values] — 1 unexpected value(s) found
  ✓ fct_orders [expression]
  ✓ fct_orders.customer_id [relationships]
  ✓ fct_orders [row_count_range] — row count: 15000

  Result: 5 passed, 0 failed, 1 warned, 0 errored
```

Filter to a single model with JSON output:

```bash
rocky test --declarative --models models/ --model fct_orders --output json
```
...
:::note
Declarative tests run against the **warehouse**, not DuckDB. They require a valid `rocky.toml` with adapter configuration and assume the target tables already exist. Use standard `rocky test` (without `--declarative`) for local, credential-free testing during development.
:::

See [Testing and Contracts — Declarative tests](/concepts/testing/#declarative-tests-tests) for the full test type reference and sidecar format.
