...
- [03-ai](03-ai/): AI generation, sync, test generation, trust-arc 5 schema-grounded validation, MCP data-grounding, agent-policy testing, the fulfillment walking skeleton (8 POCs · `ANTHROPIC_API_KEY` for 01–05, DuckDB for 06–08; 08's `run-live.sh` also uses `ANTHROPIC_API_KEY`)
...
- [05-orchestration](05-orchestration/): Hooks, webhooks, state, resume, Valkey cache, trust-arc 3 circuit breaker, idempotency keys, state retention sweep, per-client state namespacing (12 POCs · DuckDB / docker)
