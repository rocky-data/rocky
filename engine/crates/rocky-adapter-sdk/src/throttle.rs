//! Adaptive concurrency throttle (AIMD).
//!
//! Generic rate-limit feedback primitive for adapters that talk to
//! rate-limited remote APIs. Lives in the SDK so `run.rs` and any future
//! warehouse adapter can consume it without depending on a specific
//! adapter crate.
