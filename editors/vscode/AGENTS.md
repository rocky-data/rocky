...
- Commands grouped by concern under `src/commands/` (AI, branch, compile, hooks, info, inspect, lineage, migration, ops, preview, run, storage, test, plus the `commandPalette` entry point and 2 inline code-lens commands)
- Code lens (inline run/compile above models)
- Activity bar sidebar views (Get Started, Extension Info, Models, Runs, Sources, Schema, Previews, Branches, Help) — empty-state copy is gated on the `rocky.hasProject` context flag so a workspace without a `rocky.toml` shows orientation instead of CLI errors
...
├── types/rockyJson.ts    # Type-alias shim — pure re-export of types/generated/ (no hand-written interfaces)
...
- Dev: `typescript ^6.0.3`, `@vscode/vsce ^3.9.1`, `mocha ^11.7.6`
- (pins drift — check `package.json` before quoting a version)
...
# `just record-demo <scenario>` from the monorepo root; output in recording/out/.
# (see recording/README.md for the gotchas).
```

## Extension Commands

`package.json` (`contributes.commands`) is the source of truth.
...
Git conventions are in the monorepo-root `AGENTS.md`.
