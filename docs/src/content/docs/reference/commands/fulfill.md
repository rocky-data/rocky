...
Be precise about what "acting as your user" means, because it is a property of how you deploy, not of Rocky. `.rocky/fulfillment/<product>/outbox/` is the one part of that directory the worker is meant to write: elicitation writes `candidate_spec.toml` and, optionally, `questions.json` there directly, and drafting's `draft_model` tool mirrors its draft there. The loop deletes and recreates `outbox/` before every dispatch. A worker meant to write only that directory needs an inherited default, such as a umask or an ACL on its parent, not a one-off `chmod` the next dispatch would erase. The loop reads the outbox only after the worker exits and its process group is killed.
...
- The rest of `.rocky/fulfillment/<product>/` except `outbox/` — task transcripts, the committed lowering manifest, and the commit protocol's own transient staging files (`phase-staging.json`, `.ff-staged`, `.ff-prev`).
...
Naming `models/` as the worker's one writable directory is not enough by itself: the state store above lives inside it by default, at `models/.rocky-state.redb`, unless you move it out with `--state-path` ([#2169](https://github.com/rocky-data/rocky/issues/2169) tracks refusing that layout by default). A worker that can write only `models/`, the outbox, and nothing else on this list, including a relocated state store, is the shape the gates are built for. A worker that can also write anything else on this list is the hostile-local-process case, and nothing here defends against it. If your worker and your runner share a user, treat that as the concession it is.
