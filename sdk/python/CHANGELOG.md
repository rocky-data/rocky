...
- **`DriftAction`, `AnomalyResult` and `AnomalyEvaluation` gain `asset_key`.** The Dagster-style asset key path, the same convention `MaterializationInfo`/`TableError` already carry, so a consumer can key off it without re-deriving it from `table`. Added to the hand-written runtime models and the generated ones; defaults to an empty list, so an engine that does not emit the field still parses. (#2073)
