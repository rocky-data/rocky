...
def test_parse_discover_with_adapter_namespaced_metadata():
    """FR-002: Rocky users integrating with Fivetran can now read
    ``SourceInfo.metadata.<field>`` directly instead of re-parsing
    connector-type strings downstream. The field is optional and empty
    on wire for adapters that haven't opted in (backward-compat with
    pre-FR-002 fixtures).
    """
    payload = {
        "version": "1.14.0",
        ...
        "sources": [
            {
                "id": "conn_ads",
                "components": {"tenant": "acme", "region": "na", "source": "fb_ads"},
            ...
            {
                # Adapter that hasn't opted in — metadata absent on wire.
                "id": "raw__orders",
                "components": {"source": "orders"},
                "source_type": "duckdb",
                "tables": [{"name": "orders"}],
            },
        ],
    }
    result = DiscoverResult.model_validate_json(json.dumps(payload))

    ads = result.sources[0]
    assert ads.metadata["fivetran.service"] == "facebook_ads"
    assert ads.metadata["fivetran.connector_id"] == "conn_ads"
    assert ads.metadata["fivetran.schema_prefix"] == "fb_ads"
    # Rich structures flow through verbatim — note the semantic rename
    # from Fivetran's wire name `reports` to Rocky's
    # `fivetran.custom_reports` (the custom_reports list contrasts with
    # the stock tables that ad-platform connectors also ship).
    assert ads.metadata["fivetran.custom_reports"][0]["name"] == "custom_report_revenue"
    assert ads.metadata["fivetran.custom_tables"][0]["table_name"] == "ads_insights"

    # Adapter that didn't opt in defaults to an empty dict rather than
    # raising or yielding ``None`` — translators/components iterating
    # `source.metadata.items()` don't need a None-guard.
    ducky = result.sources[1]
    assert ducky.metadata == {}


def test_generated_source_output_exposes_metadata():
    """FR-002: the autogenerated Pydantic ``SourceOutput`` model (from
    ``schemas/discover.schema.json``) exposes the ``metadata`` field so
    ...
    src = SourceOutput.model_validate(
        {
            "id": "conn_1",
            "components": {"source": "fb_ads"},
            "source_type": "facebook_ads",
            "tables": [],
            "metadata": {"fivetran.service": "facebook_ads"},
        }
    )
    assert src.metadata == {"fivetran.service": "facebook_ads"}

    # Backward-compat: older fixtures without ``metadata`` still parse.
    src_no_meta = SourceOutput.model_validate(
        {
            "id": "conn_2",
            "components": {"source": "orders"},
            "source_type": "duckdb",
    ...
    assert src_no_meta.metadata is None
