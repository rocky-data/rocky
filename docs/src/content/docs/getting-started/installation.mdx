...
That is the install done. Next, run the [Quickstart](/getting-started/quickstart/). No warehouse handy? The [Playground](/guides/playground/) runs the same flow on a local DuckDB file, with no credentials needed.
