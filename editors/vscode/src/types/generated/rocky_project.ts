...
export interface ChecksConfig {
   ...
   *
   * Must be a finite number. TOML accepts `nan` and `inf`, and a NaN threshold disabled detection silently: every comparison with NaN is false, so nothing was flagged and every count was called "within normal range" (#1816). A non-finite value is refused when the config loads; `0` is the way to turn detection off, and it says so in the result.
   */
