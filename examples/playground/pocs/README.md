...
101 small POCs across 8 categories. Each is self-contained; `cd` into a POC folder and run `./run.sh`.
...
- [02-performance](02-performance/): Incremental, merge, partition checksum, column propagation, optimization, drift, view intermediate, delete+insert, adaptive concurrency, trust-arc 2 cost+budgets, strategy showcase, replication table overrides, EXPLAIN cost estimation, skip-unchanged gate (14 POCs · DuckDB)
- [03-ai](03-ai/): AI generation, explain bootstrap, sync, test generation, trust-arc 5 schema-grounded validation, MCP data-grounding, agent-policy testing, the fulfillment walking skeleton (8 POCs · `ANTHROPIC_API_KEY` for 01–05, DuckDB for 06–08; 08's `run-live.sh` also uses `ANTHROPIC_API_KEY`)
...
- [05-orchestration](05-orchestration/): Hooks, webhooks, state, resume, Valkey cache, Dagster DAG mode, trust-arc 3 circuit breaker, idempotency keys, state retention sweep, per-client state namespacing, signed webhook ingress (12 POCs · DuckDB / docker)
...
| POCs | Credentials |
|---|---|
| 89 of 101 | None — local DuckDB (or docker-compose for MinIO / Valkey / Trino) |
