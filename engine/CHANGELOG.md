...
- **Canonical Fivetran state envelope** (`FivetranStateEnvelope`) in `rocky-fivetran::envelope`. New typed shape — `version` (newtype enum, wire form `"1.0"`), `fetched_at`, `destination`, `connectors` (sorted by id at construction time via `from_parts`), `schemas` (`BTreeMap` keyed by connector_id) — derives `JsonSchema`, so the Pydantic and TypeScript bindings autogenerate via the existing `just codegen` cascade. Sibling helper `envelope_hash` excludes `fetched_at` from the hash input so two envelopes capturing the same upstream state at different wall-clock instants hash equal — the property the idempotent emit-to-path contract relies on.
- **`rocky discover --emit-fivetran-state-to <PATH>`** (FR-C). Writes a canonical envelope to `<PATH>` for every `[adapter.fivetran]` declared in the project config. Single-adapter configs write to `<PATH>` directly; multi-adapter configs write to `<PATH>.<account_hash>.<destination_id>.json` per adapter (where `account_hash` is the SHA-256-truncated hash also used to namespace the FR-B per-host rate-limit budget). The write is idempotent: a sibling `<PATH>.blake3` file records the content hash; when the freshly-computed hash matches, both files are left untouched so downstream `stat(2)` watchers see a stable mtime. Tmp + fsync + rename for crash safety.
- **`FivetranClient::fetch_envelope(destination_id, refresh)`** — in-process memoization of the per-destination envelope, keyed by `destination_id`. `refresh = false` returns the memoized value when present; `refresh = true` always forces a fresh fan-out across `GET /v1/destinations/{id}`, `GET /v1/groups/{id}/connectors`, and `GET /v1/connectors/{conn_id}/schemas` (parallelized via `buffer_unordered(10)`). Net-new `get_destination` HTTP path; the prior `FivetranClient` only fetched destinations indirectly.

### Notes

- **Cross-process / persistent cache layers are tracked separately.** PR-1 ships only the in-process memoization. The pluggable cache backend (`NoCache` / `FileCache` / `ObjectStoreCache` / `ValkeyCache` / `TieredCache`) and the `[adapter.fivetran.cache]` config block land in a follow-up.
