...
Every `--output json` command emits a struct with `version` + `command` fields plus a command-specific payload. There are **29 schemas** total; full list in `engine/CLAUDE.md` → "JSON Output Schema". Highlights:
