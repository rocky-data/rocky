...
- **Pluggable persistent state cache for the Fivetran adapter** (FR-A). New `FivetranStateCache` trait in `rocky-fivetran::state_cache` plus five concrete backends:
...
- **`[adapter.<name>.cache]` config block** under any `type = "fivetran"` adapter. Fields: `backend` (one of `"none"` / `"file"` / `"object_store"` / `"valkey"` / `"tiered"`, defaults to `"none"`), `file_root`, `object_store_url`, `valkey_url`, `valkey_ttl_seconds`. Cross-field validation runs at `load_rocky_config` time (e.g. `backend = "tiered"` requires both `object_store_url` and `valkey_url`) and surfaces missing fields as a typed `ConfigError::FivetranCacheMissingField` so the operator gets a clear error before the first cache touch. All string fields support the existing `${VAR}` / `${VAR:-default}` env-var substitution.
- **`rocky discover --no-cache`** — skip the persistent cache read on the current invocation and force a fresh API fetch. The fresh envelope is still written back to the cache so the next invocation sees the up-to-date data. Useful after rolling a Fivetran credential or when an operator suspects the cache is stale.
- **`FivetranClient::with_state_cache(Arc<dyn FivetranStateCache>)`** — builder method to plumb the configured backend into the client. Construction is wired at the two `FivetranClient::with_retry` call sites (`rocky-cli::registry` and `rocky-cli::commands::discover::emit_fivetran_state`).
- **OTLP span events for cache decisions**, mirroring the FR-B `fivetran.rate_limit_observed` emission pattern. `fivetran.cache_hit` (`backend`, `key`, `age_seconds`), `fivetran.cache_miss` (`backend`, `key`, `reason` ∈ {`"no-entry"`, `"refresh-forced"`}), `fivetran.cache_write` (`backend`, `key`, `bytes`, `outcome = "written"`), `fivetran.cache_write_skipped` (`backend`, `key`, `reason = "hash-match"`), `fivetran.cache_write_failed` (`backend`, `key`, `error`). All events are also published on the in-process pipeline event bus so sensor / orchestrator code that subscribes to `PipelineEvent` sees the same signal as the OTLP exporter.
