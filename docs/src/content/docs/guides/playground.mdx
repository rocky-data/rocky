...
The `ci` command runs compilation and testing in one pass, and returns an exit code your CI job can branch on:
...
Exit code 0 means every check passed. A non-zero exit code fails the CI job.
