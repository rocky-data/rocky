...
- **`row_count_anomaly` is no longer green when the detector never ran.** The check was in `PASS_BY_ABSENCE_CHECK_NAMES` on the reasoning that the engine reports an anomaly only when it has one, so silence is clean. That holds only if the producer is unconditional, and this one is not: with `row_count = false`, no state store, an unmeasured count or an unreadable history, no detector runs and the check was badged green anyway (#1790). The engine now reports per table whether it evaluated one (`RunResult.anomaly_evaluated`), so an evaluated table with no anomaly passes, and a table the detector skipped reports `passed=False` with severity WARN, `status="not_evaluated"` and the engine's reason. A table with an anomaly keeps its existing failing result. **On upgrade: a check that was green can turn amber** — on any pipeline where the detector was not running, and against an engine too old to send the new field. `compliance_exception` stays a pass-by-absence check: a crashed `rocky compliance` already yields an explicit not-evaluated result rather than nothing. (#1790)
...
- **`init_rocky_project()` writes a `rocky.toml` the engine accepts.** The scaffold's template used the top-level `[source]`, `[warehouse]`, `[target]`, `[replication]` and `[checks]` tables that no current engine reads: `rocky validate` failed on it with `V001 unknown field 'checks'`, so a scaffolded project could never validate or run. Its header and README also said `rocky run --config rocky.toml`, which exits 2 because `--config` is a global flag and belongs before the verb. The template now declares `[adapter]` plus `[pipeline.main]`, a `full_refresh` copy of every `src__*` schema with `row_count` and `column_match` checks, so no source column is required; the incremental strategy and the freshness check stay in the file as a commented opt-in that names the load-timestamp column they need. Validated against engine 1.74.0 and run twice on a seeded DuckDB source with matching counts. Both command lines put `--config` first; the README's install one-liner points at `engine/install.sh` on `main`, because the release-asset URL it used returns 404, and its Fivetran link points at the docs site instead of a README that does not exist. The scaffold also writes a `.gitignore` for the DuckDB file, `.rocky/` and the state store that the first run creates. (#1991)
- **A Pipes `drift` check result now arrives as an `AssetObservation`, matching the streaming path.** `drift` (`DRIFT_CHECK_NAME`) is intentionally never a declared check spec, so left to the generic undeclared-check handling it would fall into `_undeclared_check_observation`'s "declared specs are stale, refresh the state" warning — the wrong diagnosis for a check that was never meant to be declared. The new `_drift_pipes_result_to_observation` converts it instead: `rocky/drift_action` / `rocky/drift_reason` / `rocky/drift_table` metadata, same as `drift_observations` on the streaming side (run-level `rocky/drift_tables_checked` / `rocky/drift_tables_drifted` aggregates aren't included — not available on a single Pipes check message). (#2073)
- **`execution_mode="pipes"` now actually delivers its events, and stops printing a misleading warning on every run — three engine-side bugs (#2163, #2159, #2166); no source change here for any of them.** All three meant every Pipes-launched run either did nothing over the wire, crashed decoding the first message, or logged "[pipes] did not receive any messages from external process" even on a run where everything else worked — see the engine CHANGELOG for the full detail. Practically: before upgrading the engine, `run_pipes()` and `RockyComponent(execution_mode="pipes")` looked idle or failed for reasons that had nothing to do with your `rocky.toml`; a plain `run_pipes()` call on an asset with no declared check specs went silently green with nothing reported, and a `RockyComponent` in pipes mode failed its step on every run because none of its declared checks ever got an answer. After upgrading the engine, real materializations and check results arrive, a run failure you were not previously seeing may now surface, and that warning stops appearing on a healthy run. This package's own test suite gained three tests in `test_pipes_mode.py` that drive Dagster's real message-decoding path (not a mock) against message streams captured from real engine launches, to catch a regression on any side of that wire format again. (engine #2163, #2159, #2166)
...
- **A check result Rocky produced but the component never declared no longer fails the step in `pipes` mode, and no longer vanishes in `streaming` mode.** The two paths did different things with the same input. `streaming` dropped the result without a word. `pipes` handed it to Dagster, which raised `DagsterInvariantViolationError` — "Received unexpected AssetCheckResult" — and failed the whole step. Adding `[checks.freshness]` to `rocky.toml` without refreshing the component state was enough to reach it.
  ...
  Both paths now filter on the same `(asset_key, check_name)` set. A result with no declared spec is logged as a warning that names the asset, the check, the verdict and the likely cause, and its verdict is recorded as an `AssetObservation` on the asset. An observation cannot gate a run and no asset-check alert fires on it — declare the spec for that — but the value is on the asset's timeline instead of nowhere. Refresh the component state after editing `[checks]` in `rocky.toml`; a non-default check also needs `surface_configured_checks: true`. (#1673)
...
- **A project whose `rocky discover` state reports no `[checks.freshness]` no longer shows a `freshness` check on its assets. It used to show one, and it was green.** The engine emits a `freshness` result only when `rocky.toml` declares `[checks.freshness]`. `RockyComponent` declared the check spec on every asset anyway, so on every materialized table the placeholder pass reported `freshness` as `passed=True` with the status "not produced by rocky". The badge was green for a check that never ran. The spec is now declared only when `rocky discover` reports a freshness config — the same condition that already decides the pipeline-level Dagster `FreshnessPolicy`. A per-model `[freshness]` frontmatter still gives that one asset a `FreshnessPolicy`; it never produced a `freshness` check result, so it does not bring the check back.

  **This removes a check from your assets.** Anything that selects `freshness` by name — an asset-check selection, a job, an alert, a sensor — stops matching on a project that never configured freshness. Add `[checks.freshness]` to `rocky.toml` to get the check back, and refresh the component state so `rocky discover` reports it.
...
- **A failing `rocky dag` no longer loads the legacy graph in its place.** `_dag_payload` caught every exception and omitted the DAG slot, and the loader read a missing slot as "this cache predates DAG mode" and fell back to the discover-based graph. That fallback is not a degraded version of the real graph — it has different assets and different dependencies — so a project whose DAG could not be built got a code location that loaded, looked plausible, and materialized a plan the project does not describe, behind a log warning.
  ...
  The write now records **provenance** for the DAG slot (`dag_status`: `success` / `failed` / `disabled`), and the loader branches on it. A recorded failure refuses. So does a state written with `dag_mode` **off** and then loaded with it on — the state key is `RockyComponent[<config_path>]` and does not include `dag_mode`, so turning the mode on does not by itself invalidate the cache, and treating that state as a pre-DAG payload reintroduced the same wrong-graph bug by another route. Under `strict_build` the write re-raises instead of persisting, matching the discover slot: in `dag_mode` the DAG slot *is* the asset graph, not one of the best-effort augmentation slots. A `dag_status` the writer never emits — an unrecognised value, a non-string, or a key present with a JSON null — also refuses rather than being read as "no failure recorded"; only a genuinely absent key is treated as legacy.

  **Two limits worth stating.** A state written by an earlier version carries no `dag_status`, and "predates DAG support" and "the old writer swallowed a DAG failure" are indistinguishable on disk — so an existing deployment whose cached `rocky dag` was already failing keeps falling back until its state is rewritten, at which point the ambiguity disappears permanently. `strict_build` refuses that ambiguous case rather than tolerating it. And the triggering condition is narrower than "any multi-pipeline project": `rocky dag` refuses when two transformation pipelines claim the **same model**, which is what the SDK's invocation produces for any project with two or more of them, because `RockyClient.dag()` always passes `--models` and the engine clones that one model list into every transformation pipeline (#1348). (#1341)
...
- **Floor raised to `rocky-sdk>=0.7.0`.** Picks up the SDK's generated types for `rocky replay --execute` and `rocky restore` plan + apply so a `dagster-rocky` consumer resolves them through `dagster_rocky.types` without relying on latest-SDK resolution. No behavior change in the integration itself.
- Internal resource efficiency and health-probe pass (no public API change). (#1101)
...
- **Migrate `RockyResource` from `rocky run` to canonical `rocky plan` + `rocky apply <plan-id>` flow** (Cluster 3 B Phase 5). The three execution methods — `run(...)`, `run_streaming(...)`, and `run_pipes(...)` — now invoke `rocky plan` first (buffered, single-digit seconds), capture the persisted `plan_id`, then dispatch `rocky apply <plan_id>` via the existing process model for that method (buffered / streamed-to-`context.log` / Dagster Pipes). Every Dagster materialization now writes an auditable plan artifact at `.rocky/plans/<plan_id>.json` — the same id a `rocky plan` CLI invocation would produce. The public method signatures and return types are unchanged (`run()` and `run_streaming()` still return `RunResult`; `run_pipes()` still returns `PipesClientCompletedInvocation`). Notable subtleties:
  - **Replication-only fallback.** Projects without a `models/` directory next to the config cannot persist a plan (`rocky plan` emits `plan_id = null`); the integration falls back to the legacy single-subprocess `rocky run` argv automatically — behavior-preserving for those projects.
  - **`run_pipes` extras wiring.** When a `plan_id` is persisted, it is also surfaced as `extras={"plan_id": ...}` on the `PipesSubprocessClient.run` call so Dagster shows the plan id as run metadata.
  - **`run_streaming` log narrative.** Plan-phase stderr is forwarded to the `dagster_rocky` module logger only (not to `context.log`); apply-phase stderr streams to `context.log` exactly as before. Operators watching the run viewer for discover/drift progress will see those lines start when the apply step starts.
  - **`ROCKY_SUPPRESS_DEPRECATION=1` kept.** The replication-only fallback still invokes `rocky run`, so the deprecation-suppress env-var on the per-subprocess env stays in place. Operators can override via the env var as before.
  - **Engine compatibility.** Requires engine `v1.32.0` or newer (where `rocky plan` + `rocky apply <plan-id>` ship with full flag-surface parity). `MIN_ROCKY_VERSION` stays at `1.0.0`; the parity check is unchanged.
- **Suppress engine deprecation notices on every subprocess invocation.** Companion to engine Phase 4 (alias deprecation for `rocky run` / `rocky branch promote <name>`). `RockyResource._run_rocky_with_log_sink` passes a per-subprocess environment to `subprocess.Popen` with `ROCKY_SUPPRESS_DEPRECATION=1` set (`setdefault`, so operators can override). Without this, every dagster-driven `rocky run` (now reduced to the replication-only fallback path, per the entry above) would emit a `[deprecated]` line into stderr and surface as Dagster log noise.
...
- **`ImportDbtOutput.emission`** (engine [#428](https://github.com/rocky-data/rocky/pull/428)). New `Optional[ImportDbtEmission]` field on the regenerated `ImportDbtOutput` Pydantic model carrying `out_dir`, `rocky_toml_path`, `migration_notes_path`, `models_translated_count`, `models_skipped_count`, `seeds_copied_count`, `adapter_type`, `original_dbt_adapter_type`, and `required_env_vars`. Populated when `rocky import-dbt` is invoked with `--output-dir`; `None` for the legacy validation-only mode. Strictly additive — re-exported from `dagster_rocky.types` for the soft-swap import path.
...
- **`RockyResource.branch_approve(...)` + `RockyResource.branch_promote(...)`** (engine [#388](https://github.com/rocky-data/rocky/pull/388)). Two new methods on `RockyResource` that drive the engine's `rocky branch approve` and `rocky branch promote` verbs and return parsed `BranchApproveOutput` / `BranchPromoteOutput`. `branch_promote(name, skip_approval=False, ...)` surfaces the file-based approval gate in the typed return — consumers branch on `result.approvals_used` / `result.approvals_rejected` / `result.audit` / `result.success` without re-parsing the CLI JSON. `skip_approval=True` passes `--skip-approval` to bypass an enabled `[branch.approval]` gate and stamps an `ApprovalSkipped` audit event. Tested in `tests/test_branch_approval.py` against fixture round-trips covering the approved-and-promoted, unapproved-blocked, and skip-approval paths.
