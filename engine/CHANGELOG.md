...
- **Corrected `rocky gc` / `rocky restore` documentation that overstated recoverability, physical deletion, and author-identity enforcement.** Three classes of claim were wrong across CLI help, doc-comments, JSON output schemas, docs, and the README. *Restorability:* the gc plan/apply notes, `[gc]` config reference, and docs told users an evicted artifact was "always restorable from the recorded recipe" and that `derivable` meant "provably rebuildable" — neither holds for a multi-input recipe, since gc's eligibility check admits a `strong` closure over several content-hashed upstreams but `rocky restore` refuses any recipe with recorded upstreams, so gc can evict artifacts restore cannot rebuild; `derivable` is now described as an eligibility verdict (a recipe was recorded and bound to the artifact's exact bytes) rather than a rebuild guarantee, and restore's supported recipe shape is stated as necessary rather than sufficient (a supported recipe can still refuse on a missing provenance binding, undeserializable canonical IR, unreachable storage, a hash that no longer reproduces, a path outside the storage prefix, or a lost reinstatement race). *Physical deletion:* the gc module docs, `PlanKind::Gc` doc, and the `physical_reclaimed` schema field implied `rocky apply` performs a "best-effort physical object-store delete" — it performs none; eviction is ledger-only (tombstone + retired ledger row) and `[gc] physical_delete = true` is a hard error, so `physical_reclaimed` is now documented as currently always `false`. *Author-identity enforcement:* the global `--principal` help and the gc/restore/`plan` doc-comments said a later `rocky apply` "evaluates the plan against the identity that authored it" — it does not read the stored stamp; the gate enforces `most_restrictive(apply-time runtime principal, plan-kind default)`, so an agent applier is gated as agent (a `deny agent gc` rule fires) and a machine-composed kind stays agent even under a human applier, while the stored stamp is advisory/display only. The eligibility-check enumeration also said "five" while `build_candidate` applies six — the exact-output binding check was omitted. Documentation only; no behavior change. The gc/restore asymmetry is pinned by a characterization test that drives gc's full six-check `build_candidate` admission — proving the eviction set gc actually approves includes a multi-input recipe `rocky restore` then refuses — and a new test drives the exact policy function the gc apply gate calls (`evaluate_apply_policy_with_policy`) with the `enforcement_principal(runtime)` result, over a plan whose stored stamp (`Human`) differs from the runtime applier, to assert `deny agent gc` fires for an agent applier and not for a human, proving the stored stamp is ignored. (#1201)
