...
Source metadata  ◀── Fivetran REST API / DuckDB info_schema / manual config
    ...
    ▼
rocky-compiler  →  diagnostics (E001–E033 / W001–W012)
...
- **otel.rs**: feature-gated OpenTelemetry exporter. When the engine is built with `--features otel`, `rocky apply` exports in-process metrics as OTLP via an `OtelGuard` RAII handle. Send metrics to any OTLP-compatible collector (Honeycomb, Datadog, Grafana Tempo, Prometheus via the OTel collector).
...
The [`rocky-adapter-sdk`](#rocky-adapter-sdk) crate provides the stable, versioned traits (`WarehouseAdapter`, `SqlDialect`, `DiscoveryAdapter`, `GovernanceAdapter`) plus an `AdapterManifest` that declares which traits each adapter implements.
