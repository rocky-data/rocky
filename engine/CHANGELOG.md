...
- **`rocky serve --scheduler` — a resident reconciler (experimental).** The same demand reconciler as `rocky tick`, hosted inside a long-lived `rocky serve` instead of behind an external timer: a clock-driven loop re-reads `rocky.toml` and runs what is due every `--poll-interval-seconds` (default 15, must be ≥ 1). The scheduler takes its first tick only after the server has completed its startup job sweep and bound its listener, so a scheduled run never precedes — or outlives a failed — server startup. Scheduled runs go through the persisted jobs model, so `GET /api/v1/jobs/{submission_id}` reports them and a restarted server reports honest status. The loop holds the app-level mutation permit for each tick, so a scheduled run and an API `run`/`apply` never collide on the state store — whichever is second gets a clean `409 mutation_in_progress` (or the scheduler skips the tick and re-evaluates next time). A config parse error on one tick is logged and skipped without killing the loop or running a stale schedule. On `SIGTERM`/`Ctrl-C` the server drains: it stops evaluating new demands, gives a run already in flight up to `--drain-timeout-seconds` (default 60, never beyond the run's own `timeout_minutes`) to finish before terminating it, and drains in-flight HTTP requests in the same window; a run interrupted by the drain finalizes as failed and is not retried until the next occurrence. `rocky doctor` gains a `scheduler` check (silent unless a pipeline declares a `[schedule]`): **Critical** when `.rocky/tick.lock` is held with a stale heartbeat (a wedged reconciler) and **Warning** when no tick has evaluated any schedule in more than twice the shortest cron interval (a dead timer); it reads only the filesystem and state, so it works offline and for cron-driven `rocky tick` too. Run one reconciler per project — a `rocky tick` timer or a `rocky serve --scheduler`, not both. Marked experimental while the reconciler soaks. (#1199)

- **`GET /api/v1/schedule` — a scheduler status endpoint.** A read-only snapshot of every scheduled pipeline: its cron/`after`/`freshness` configuration, when it last evaluated and fired, its projected next fire, any active backoff, the claims in flight, and the tick-lock state. The projection is anchored on the last occurrence that actually fired, so an overdue pipeline reports a `next_fire_at` in the **past** rather than a healthy-looking future one, and a `tick_lock.state` of `free` is the normal state between ticks (not a stopped scheduler). It reports stored state without evaluating demand — `rocky tick --dry-run` remains the evaluation — so it is side-effect free. A pipeline whose schedule cannot be resolved is reported with its `config_error` rather than silently omitted, and an unreadable `rocky.toml` is a `500 config_invalid` rather than an empty `200`. The scheduled set comes from the reconciler's own resolution, so load pipelines (which cannot be scheduled) are correctly excluded. HTTP-only: there is no `rocky schedule` CLI verb, but the payload is a codegen-backed `ScheduleStatusOutput` so the SDK and VS Code bindings are generated like every other output. (#1202)
- Each `rocky serve --scheduler` tick now emits a `scheduler.tick` span carrying the tick's counts (`rocky.scheduler.due`, `.executed`, `.failed`, `.skipped`, `.lock_overridden`, `.state_busy`, `.drained`) plus a `rocky.scheduler.outcome` naming why the tick ended — `completed`, or `config_error` / `permit_held` / `lock_skipped` / `fault` for the paths where the loop ticks without reconciling. The span covers the whole tick, so "the scheduler is alive but running nothing" is legible in a trace rather than showing up as an absence. Exported wherever `rocky serve`'s existing tracing goes, including OTLP when `OTEL_EXPORTER_OTLP_ENDPOINT` is set; no new configuration. Child runs still trace independently — the scheduler does not yet propagate trace context into them. (#1202)

- **A `merge` `unique_key` naming a column the model does not produce now raises a compile-time warning (W006).** Such a key previously drew no diagnostic and only surfaced when the warehouse rejected the generated `MERGE ... ON` clause. W006 is the merge-key counterpart to the E020 `time_column` existence check, but is emitted as a warning — it does not block compilation. One diagnostic is emitted per missing key, so a composite `unique_key` reports every typo in a single compile rather than one per fix-recompile round. It is an absence check: skipped when the model's output columns cannot be fully enumerated (a `SELECT *` over an upstream with no known schema), mirroring the existing `type_known` guard, and covers `strategy = "merge"` — the only `unique_key` the compiler can see. (#1200)

### Fixed

- `rocky compact apply` and `rocky archive apply` now route through the agent-policy plane like the other apply kinds, closing an enforcement-parity gap. Both sinks previously executed their warehouse SQL (`OPTIMIZE`/`VACUUM` for compact, the destructive `DELETE` + `VACUUM` for archive) without consulting `[policy]`, on both the generic `rocky apply <plan>` route and the direct `rocky compact/archive apply <plan>` alias route. They now gate under `PolicyCapability::Apply` — keyed on the plan's touched targets (physical FQN mapped back to the declaring model's logical name where possible) and evaluated against the plan's enforcement principal (the apply-time runtime identity combined most-restrictively with the plan kind, never the plan's stored stamp) — before any statement executes: a `deny` refuses the apply, a `require_review` requires the `rocky review <plan> --approve` marker, and an active cross-pod freeze is pulled before the gate reads it. `rocky review --approve` now accepts compact and archive plans (a marker-only sign-off, like gc/restore), so a `require_review` verdict on a maintenance apply can actually be cleared. A targetless archive plan (`DELETE FROM *`) cannot be gated, so it is refused outright when a `[policy]` block is configured rather than being allowed to vacuously pass an empty target set. This is enforcement parity across the apply kinds, not a hardened boundary — the policy plane remains cooperative (self-asserted principal); absent a `[policy]` block, behaviour is unchanged. (#1209)
- All three `rocky branch promote` routes now enforce the agent-policy plane on the apply-time runtime principal, closing a promote bypass. `rocky apply <promote-plan>` and `rocky branch promote --plan` already ran the policy gate before touching the warehouse, but bare `rocky branch promote <name>` reached `execute_statement` after only its approval and breaking-change checks and never consulted the policy plane — so an agent-run promote that a `deny agent promote` rule (or an active freeze) would refuse could still execute. Bare promote now honors the plan-plus-apply flow its deprecation notice already documented: it builds and persists a promote plan (running the same approval and breaking-change gates as before) and routes execution through the shared promote gate, so a denied agent promote is refused before any statement runs. All three routes reach the identical verdict on the same promote, keyed on the runtime principal and the plan kind rather than the plan's stored (non-integrity-protected) principal stamp; a human runtime is unaffected. The bare verb's existing behavior is preserved through the delegation: its `--pipeline` selector and `--skip-approval` flag continue to work, and a breaking-change block still writes the same `success:false` JSON to stdout (with a non-zero exit and the human-readable error on stderr). The only additive change is a `PromotePlanCreated` audit event, since the bare verb now genuinely builds a plan. (#1211)
- `rocky serve` now honors an explicit `--state-path`. It previously resolved the conventional `<models>/.rocky-state.redb` regardless, so a server started with an explicit path recorded its job records — and, with `--scheduler`, its scheduler cursors, claims, and child run history — in a different file than the one selected. (#1199)
- `rocky doctor --check scheduler` no longer reports healthy when it could not actually check: an unreadable `rocky.toml` is now a failed check rather than silence (which surfaced as `overall: healthy`, exit 0), and a tick-lock probe error or an unreadable state store is a warning rather than a green "reconciler healthy". A missing state file is still healthy — it just means no tick has run yet. (#1199)
...
- **Corrected `rocky gc` / `rocky restore` documentation that overstated recoverability and physical deletion.** Across CLI help, doc-comments, JSON output schemas, the command reference, the README, and this changelog, three classes of claim were wrong. *Restorability:* the gc plan/apply notes, the `[gc]` config reference, and the docs told users an evicted artifact was "always restorable from the recorded recipe" and that `derivable` meant "provably rebuildable" — neither holds for a multi-input recipe, because gc's eligibility check admits a `strong` closure over several content-hashed upstreams while `rocky restore` refuses any recipe with recorded upstreams, so gc can evict artifacts restore cannot rebuild. `derivable` is now described as an eligibility verdict (a recipe was recorded and bound to the artifact's exact bytes), and restore's supported recipe shape (non-partitioned, content-addressed, no recorded upstreams) is stated as necessary rather than sufficient — a supported recipe can still refuse on a missing provenance binding, undeserializable canonical IR, unreachable storage, a hash that no longer reproduces, a path outside the storage prefix, or a lost ledger-reinstatement race. A restore refusal is also clarified: it guarantees only that the ledger was not reinstated, not that nothing was written (verified content-addressed bytes may already be on disk and are safe to reuse). *Physical deletion:* the gc module docs, the `PlanKind::Gc` doc, and the `physical_reclaimed` schema field implied `rocky apply` performs a "best-effort physical object-store delete" — it performs none; eviction is ledger-only (tombstone + retired ledger row) and `[gc] physical_delete = true` is a hard error, so `physical_reclaimed` is now documented as currently always `false`. The eligibility-check enumeration also said "five" while the admission applies six — the exact-output binding check was omitted. Two further doc-only clarifications ride along: the gc/restore doc-comments describing how the gc apply gate resolves the invoker principal now match the shipped behavior (apply enforces `most_restrictive(apply-time runtime principal, plan-kind default)`, not the plan's stored stamp, which is advisory/display only), and the `[policy]` config reference no longer claims an absent block activates the default agent posture (`apply`/`promote`/MCP-write enforcement returns `NotConfigured` and allows the action when no block is present; only `rocky policy check` consults the default posture). Documentation and tests only; no behavior change. The gc/restore asymmetry is pinned by a new characterization test that drives gc's full six-check `build_candidate` admission against one multi-input artifact and asserts `rocky restore` then refuses that very artifact. (#1212)
- **Bootstrap table creation is now non-replacing, closing a data-loss class where a transient probe failure could erase a live target.** First-run CTAS across the replication, transformation, time-interval, and lakehouse paths (and the contract-gated `rocky load` promotion) previously used `CREATE OR REPLACE` whenever a target `DESCRIBE` collapsed to "target absent" — so a transient describe-probe failure misread as absence would overwrite an existing populated target. Bootstrap now fails closed on an ambiguous probe: a positively-transient describe failure is propagated (via `FailureClass`) and retried instead of collapsing to absence, and a confirmed-existing target keeps the replacing path for intentional full refreshes. A genuine not-found is never transient, so first-run bootstrap is preserved on every adapter by construction, with the non-replacing CTAS as the backstop. (#1207, #1208)
- The Trino adapter now classifies its failures for retry. It was the last warehouse adapter without a `classify_failure` implementation, so its errors fell to the `Unknown` default and were never retried — and, after the fail-closed bootstrap change above, a transient describe-probe failure on Trino would have been treated as target-absence. Trino now mirrors the Snowflake/BigQuery idiom (network / 5xx / 429 / timeout / cluster-blip → transient; SQL / user / auth / decode → permanent), erring toward permanent by construction so first-run bootstrap stays preserved. (#1210)
...
- **`rocky restore` — rebuild a gc-evicted artifact from its tombstone, hash-exact.** A gc tombstone points at the recipe that produced the evicted bytes; `rocky restore <target>` attempts to rebuild them from it and refuses unless the recomputed hash matches (a route that exists only for a recipe reading no recorded upstreams — see the correction below). It resolves the tombstone by model name, `model@<recipe-hash-prefix>`, or content hash (refusing an ambiguous target with the candidates listed, and distinguishing "never evicted" from "not found"), then writes a review-gated restore plan. `rocky apply <plan>` re-derives the artifact from the recorded recipe **on the recording engine** (the project's configured warehouse — a content-addressed artifact is only reproducible bit-for-bit on the engine that wrote it), verifies the recomputed content hash equals the tombstoned hash **before anything is written**, and only then re-materializes to the original path and reinstates the ledger row — the tombstone is kept, stamped with the restore for an auditable evict→restore history. Every failure mode is fail-closed: a recipe that no longer reproduces the exact bytes refuses and writes nothing, bytes already present with a different hash are never overwritten, and restore is symmetric-caution gated like gc (even a human restore goes through review; policy can only tighten). **Correction (see #1212):** this entry originally said restore "closes the reclaim→replay loop that makes gc eviction safe". It does not close the loop — restore covers only recipes that read no recorded upstreams, while gc's eligibility check admits multi-input recipes, so some evicted artifacts have no working restore path. The "refuses and writes nothing" clause is also not a universal: the hash-mismatch refusal is pre-write, but a refusal can also follow a successful, hash-verified content-addressed write whose atomic ledger reinstatement then lost a race — a refusal guarantees only that the ledger was not reinstated (no restore of record), and any verified bytes already at the tombstoned path are safe to reuse on re-apply. (#1103)
...
- **`rocky gc` reclamation with deletion behind review.** `rocky gc --derivable` now writes a review-gated plan (alongside the existing `--dry-run` inventory); `rocky apply <plan>` re-derives eligibility against the live ledger and evicts only artifacts it can still prove are rebuildable — matched by full identity (run, model, file path, content hash) — committing a restore tombstone and retiring the ledger row in one transaction before a best-effort physical delete. Deletion is symmetric-caution gated: even a human gc goes through review, and policy can only tighten. Eligibility is hash-exact and fail-closed, so a derivability proof for one artifact can never authorize deleting another, and a stale or hand-authored plan is refused. (#1078) **Correction (see #1212):** two clauses here overstate. No physical delete follows — eviction is ledger-only and `[gc] physical_delete = true` is a hard error; the "best-effort physical delete" describes a path that was never implemented. And "prove are rebuildable" overstates the check, which proves only that a recipe was recorded and bound to the artifact's exact bytes; `rocky restore` refuses any recipe with recorded upstreams, so gc can evict artifacts restore cannot rebuild.

### Changed

- State schema `v18 → v19`: an additive tombstones table recording each eviction's recipe identity and restore pointer. Additive — older state opens forward-compatibly. Note that the bump also rotates the schema-version-qualified **remote** state key, so a `state_sync`-backed deployment starts a fresh remote copy at the new key on its first post-upgrade sync; local state migrates in place.
