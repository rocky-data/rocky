...
Every response carries a top-level `version` field naming the engine release that produced it. Rocky sets it at compile time from the binary's own version, so `rocky --output json` always reports the version you are running. The examples on this page pin a representative version string for readability; your output shows whichever engine you installed.

**Compatibility contract:**
- New fields ship in a minor release and do not break an existing consumer.
- Removing or renaming a field is breaking, and happens only in a major release.
- Parse defensively and ignore fields you do not recognize.

The canonical machine-readable schemas live in `schemas/*.schema.json`, written by `rocky export-schemas`. Validate against those.

## Exit codes

Read the [exit code](/reference/glossary/#exit-code) alongside the JSON, because a run can be part-successful:
...
The key maps straight onto an orchestrator's own asset identifier, such as Dagster's `AssetKey`.
...
Every other `--output json` command returns the same versioned envelope. Rather than restate each shape here and let it rot, this page points you at two places that stay current:

- **Machine-readable schemas** — [`schemas/*.schema.json`](https://github.com/rocky-data/rocky/tree/main/schemas), one file per command, written by `rocky export-schemas`. These same files generate the Python and TypeScript bindings, so validate against them.
- **Worked examples** — each command's entry in the [CLI Reference](/reference/cli/) and in the category pages under [Reference → Commands](/reference/commands/core-pipeline/).

The three shapes below are the ones consumers most often get wrong. The authoritative shape for each still lives in the generated schema.
