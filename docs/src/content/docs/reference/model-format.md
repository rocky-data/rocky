...
`rocky run` records the model as a failed table and leaves its existing table alone. If an earlier run built that table, it keeps the rows those runs appended again. By default, a model downstream still builds from it. With no earlier table, that downstream model fails instead. Set `contain_failures = true` under `[resilience]` to hold back everything downstream of the failed model instead. Rebuild the table before you trust it, for example with one `full_refresh` run.
