...
# an engine/-touching PR must add an engine/CHANGELOG.md entry UNDER
# [Unreleased] specifically, or carry a `Changelog: none` marker with a real
# reason.
#
...
# fixture PR bodies, fixture `git diff` text, and fixture CHANGELOG.md files
# standing in for the HEAD (post-PR) content -- no git repo, no network, no
# GitHub Actions runner. The workflow's own live-fire check (does the
...
# Fixture CHANGELOG.md contents standing in for the HEAD (post-PR) file.
# has_changelog_entry() reads this to find the extent of the Unreleased
# section, independently of the diff text, so each diff fixture below has a
# matching "resulting file" fixture here.

readonly FIXTURE_EMPTY="$WORK/empty.md"
...
- An old, already-released entry.
EOF

readonly FIXTURE_WITH_ENTRY="$WORK/with_entry.md"
cat >"$FIXTURE_WITH_ENTRY" <<'EOF'
# Changelog
...
readonly FIXTURE_HEADER_ONLY="$WORK/header_only.md"
cat >"$FIXTURE_HEADER_ONLY" <<'EOF'
# Changelog
...
readonly FIXTURE_OLD_RELEASE_ONLY="$WORK/old_release_only.md"
cat >"$FIXTURE_OLD_RELEASE_ONLY" <<'EOF'
# Changelog
...
- Added a note to an old release retroactively. (#1938)
- An old, already-released entry.
EOF
...
# The added line sits under an already-released heading, not Unreleased --
# the case caught in review: an earlier version of has_changelog_entry
# accepted any non-blank added line anywhere in the file.
readonly DIFF_OLD_RELEASE_ONLY='diff --git a/engine/CHANGELOG.md b/engine/CHANGELOG.md
index abc123..def456 100644
--- a/engine/CHANGELOG.md
+++ b/engine/CHANGELOG.md
@@ -9,6 +9,7 @@
...
out=0
result="$(run_gate "$BODY_NO_MARKER" "$DIFF_EMPTY" "$FIXTURE_EMPTY" 2>&1)" || out=$?
...
out=0
result="$(run_gate "$BODY_NO_MARKER" "$DIFF_WITH_ENTRY" "$FIXTURE_WITH_ENTRY" 2>&1)" || out=$?
...
out=0
result="$(run_gate "$BODY_BARE_MARKER" "$DIFF_EMPTY" "$FIXTURE_EMPTY" 2>&1)" || out=$?
...
out=0
result="$(run_gate "$BODY_BARE_MARKER" "$DIFF_WITH_ENTRY" "$FIXTURE_WITH_ENTRY" 2>&1)" || out=$?
...
out=0
result="$(run_gate "$BODY_VALID_MARKER" "$DIFF_EMPTY" "$FIXTURE_EMPTY" 2>&1)" || out=$?
...
out=0
result="$(run_gate "$BODY_NO_MARKER" "$DIFF_HEADER_ONLY" "$FIXTURE_HEADER_ONLY" 2>&1)" || out=$?
...
out=0
result="$(run_gate 'Changelog: none because this only touches tests, no behaviour change' "$DIFF_EMPTY" "$FIXTURE_EMPTY" 2>&1)" || out=$?
...
out=0
result="$(run_gate 'Changelog: none - short' "$DIFF_EMPTY" "$FIXTURE_EMPTY" 2>&1)" || out=$?
...
out=0
result="$(run_gate 'CHANGELOG: NONE - this reason is definitely long enough' "$DIFF_EMPTY" "$FIXTURE_EMPTY" 2>&1)" || out=$?
...
out=0
crlf_body=$'## Summary\r\nChangelog: none - test-only refactor, no behaviour change\r\n'
result="$(run_gate "$crlf_body" "$DIFF_EMPTY" "$FIXTURE_EMPTY" 2>&1)" || out=$?
...
out=0
result="$(run_gate "$template_body" "$DIFF_EMPTY" "$FIXTURE_EMPTY" 2>&1)" || out=$?
...
out=0
result="$(run_gate "$BODY_NO_MARKER" "$DIFF_OLD_RELEASE_ONLY" "$FIXTURE_OLD_RELEASE_ONLY" 2>&1)" || out=$?
...
echo "changelog-gate decision logic: 13 cases ok"
