...
- **`rocky-lang`: public `parse_all` helper for parallel corpus parsing.** Exposes a rayon-backed `parse_all(&[S: AsRef<str>]) -> Vec<Result<RockyFile, ParseError>>` that parses a corpus of `.rocky` sources in parallel while preserving input order and never short-circuiting on the first error. Below a small `PAR_THRESHOLD` (32, mirroring `rocky-compiler/typecheck.rs::with_min_len(32)`) it falls back to a sequential pass to avoid rayon's worker-dispatch overhead — `.rocky` parsing is ~1-2 µs per typical model on an M-series core, and small corpora otherwise regress under `par_iter`. The pre-existing parallel parse-and-lower loop in `rocky-compiler::project::load_rocky_models` is unchanged; the new helper centralises the par-iter strategy for read-only corpus callers (LSP, AI, future incremental front-ends) so the threshold lives in one place. New `engine/crates/rocky-lang/benches/parse_corpus.rs` criterion bench measures `parse_all` against a sequential `.iter().map(parse)` baseline at N=10/50/200, providing a regression guard if `par_iter` ever flattens against sequential. `rayon` lifted to `[workspace.dependencies]` to replace the two existing `rayon = "1"` declarations in `rocky-core` and `rocky-compiler`.
