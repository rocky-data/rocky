...
<CardGrid>
  ...
  <Card title="A tested exit door, not a one-way door" icon="cloud-download">
    `rocky emit-sql` renders the compiled SQL for your transformation models with no warehouse connection. The output is plain runnable SQL with no Rocky-specific syntax, so adopting Rocky never locks you in. A CI test emits the full-refresh SQL and runs it directly against DuckDB, so the exit path stays runnable as the engine evolves.
  </Card>
</CardGrid>
