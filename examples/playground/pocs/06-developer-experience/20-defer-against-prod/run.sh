...
# Use the freshly-built engine binary by default (the smoke runner's bare
# `rocky` on PATH is often a stale install that predates --defer). Override
# with ROCKY_BIN=... to point at a different binary.
