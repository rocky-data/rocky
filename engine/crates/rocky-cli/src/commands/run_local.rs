...
"#,
    ...
    // -----------------------------------------------------------------
    // Pipes `opened`/`closed` coverage for the three run_local.rs paths
    // (#2166). `run.rs`'s own dispatch already called `closed()` on its
    // two paths; these three (`run_transformation`, `run_quality`,
    // `run_snapshot`) each detect their OWN `PipesEmitter` independently
    // and never called it, so a real Dagster launch through any of them
    // never got a `closed` message and the last-message-wins bookkeeping
    // a `PipesCompositeMessageReader` relies on stayed wrong. Real
    // end-to-end coverage, not a hand-built message: each test runs the
    // actual pipeline function against a temp DuckDB file with zlib+base64
    // -encoded env vars — the same shape `dagster_pipes.encode_param`
    // produces — and reads back the real file the emitter wrote.
    // -----------------------------------------------------------------

    // `cargo test` runs this crate's tests in parallel by default, and
    // these tests mutate process-global DAGSTER_PIPES_CONTEXT /
    // DAGSTER_PIPES_MESSAGES. Shared lock, not a per-file one — see
    // `crate::testing::PIPES_ENV_LOCK`'s doc comment for why a per-file
    // lock isn't enough (this module's own tests are the case that
    // proved it).
    use crate::testing::lock_pipes_env as lock_env;

    /// `base64(zlib(json))` — what every real `dagster_pipes.encode_param`
    /// call produces, and the only shape `PipesEmitter::detect` accepts
    /// (#2163). Duplicated from `pipes::tests::encode_like_dagster_pipes`
    /// rather than shared across files, matching this file's existing
    /// `ENV_LOCK` convention.
    ...
    /// Sets both Pipes env vars for the duration of `f`, restores whatever
    /// was there before (even if `f` panics — the mutation-check for these
    /// three tests removes the `closed()` call, which does not panic, so
    ...
    async fn run_with_pipes_capture<T, Fut: std::future::Future<Output = T>>(
        ...
        let prior_context = std::env::var(crate::pipes::ENV_PIPES_CONTEXT).ok();
        let prior_messages = std::env::var(crate::pipes::ENV_PIPES_MESSAGES).ok();
        // SAFETY: serialised by ENV_LOCK.
        unsafe {
            std::env::set_var(crate::pipes::ENV_PIPES_CONTEXT, &context_env);
            std::env::set_var(crate::pipes::ENV_PIPES_MESSAGES, &messages_env);
        }
        ...
        unsafe {
            match prior_context {
                Some(v) => std::env::set_var(crate::pipes::ENV_PIPES_CONTEXT, v),
                None => std::env::remove_var(crate::pipes::ENV_PIPES_CONTEXT),
            }
            match prior_messages {
                Some(v) => std::env::set_var(crate::pipes::ENV_PIPES_MESSAGES, v),
                None => std::env::remove_var(crate::pipes::ENV_PIPES_MESSAGES),
            }
        }
    ...
    async fn run_transformation_pipes_path_opens_and_closes() {
        ...
        seed_src(&db, 3).await;
        write_model(&models_dir, "stg", "SELECT id FROM main.src", &[]);
        write_config(dir.path(), &db, "");

        let (lines, ()) = run_with_pipes_capture(&messages_path, || {
            run_full_dag(&config_path, &state_path, false, None, None)
        })
        .await;
...
"#,
        ...
        // Not `run_full_dag`: DuckDB's `MERGE ... WHEN NOT MATCHED THEN
        // INSERT (*) VALUES (source.*, ...)` (the SQL `generate_snapshot_sql`
        // emits for this pipeline type) is a syntax DuckDB's parser refuses
        // — a pre-existing dialect gap, unrelated to Pipes and out of scope
        // here. `closed()` must still fire on THIS failure path (it runs
        // before the `tables_failed > 0` bail in `run_snapshot`), which is
        ...
        let opts = PartitionRunOptions::default();
        let defer_opts = DeferOptions::default();
        let skip_opts = SkipRunOptions {
        ...
        let (lines, dispatch_result) = run_with_pipes_capture(&messages_path, || {
            super::super::run::run(
                &config_path,
                std::sync::Arc::new(
                    rocky_core::config::load_rocky_config_fingerprinted(&config_path).unwrap(),
                ...
                None, // filter
                None, // pipeline_name_arg
                &state_path,
                None,  // governance_override
                false, // output_json
                None,  // model filter (not a transformation pipeline)
                false, // run_all
                None,  // resume_run_id
                false, // resume_latest
                None,  // shadow_config
        ...
        let err = dispatch_result.expect_err(
        ...
        assert!(
            ...
            "expected the known DuckDB-dialect failure, got a different error \
             (a real regression?): {err}"
        );
