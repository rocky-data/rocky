...
Rocky is a monorepo with four shipping subprojects + a playground catalog. Most non-trivial changes cross subproject boundaries, and the cascade rules are easy to forget. Use this skill as a router.

## The subprojects at a glance

| Path | Language | Ships as | Build | Test |
|---|---|---|---|---|
| `engine/` | Rust | `rocky` + `rocky-lsp` binaries | `cargo build --release` | `cargo test` |
| `sdk/python/` | Python | `rocky-sdk` wheel | `uv build` | `uv run pytest` |
...
Each has its own `AGENTS.md` with subproject-specific coding standards and commands. **Read the subproject `AGENTS.md` before editing it** — they're kept current.
...
→ **`rocky-dsl-change`** skill. Lockstep change across `engine/crates/rocky-lang`, `engine/crates/rocky-compiler`, `editors/vscode/syntaxes/rocky.tmLanguage.json`, `editors/vscode/snippets/rocky.json`, `docs/src/content/docs/concepts/rocky-dsl.md`, and `docs/rocky-lang-spec.md`.
...
→ Use the engine-local `databricks`, `fivetran`, or `rocky` skill when the active agent client exposes it. If it is not available, start with `engine/AGENTS.md` and the referenced adapter source and API documentation.

### "Something Dagster-specific"
→ Also use an up-to-date Dagster specialist skill when one is available in the active agent client. The integration at `integrations/dagster/` leans on Dagster's current component/pipes/asset model, which evolves quickly.
...
|---|---|
| `engine/crates/rocky-cli/src/output.rs` (any `*Output` struct) | Run `just codegen` — auto-regenerates `schemas/`, `sdk/python/src/rocky_sdk/types_generated/`, `editors/vscode/src/types/generated/`. Then in `sdk/python/src/rocky_sdk/types.py`: re-export the new type and add a `parse_rocky_output()` dispatch entry (the hand-written runtime models there are gated by `test_schema_parity.py`, not codegen); `dagster_rocky.types` re-exports from `rocky_sdk`. |
| A `.rocky` keyword or operator | `engine/crates/rocky-lang/src/{token,parser,lower}.rs`, `engine/crates/rocky-compiler/src/`, `editors/vscode/syntaxes/rocky.tmLanguage.json`, `editors/vscode/snippets/rocky.json`, `docs/src/content/docs/concepts/rocky-dsl.md`, `docs/rocky-lang-spec.md` |
...
- `AGENTS.md` (root) — monorepo-wide cascade rules
- `engine/AGENTS.md` — engine coding standards, JSON output schema table, Databricks SQL patterns, auth flows
- `integrations/dagster/AGENTS.md` — dagster layer architecture, "Adding support for a new Rocky CLI command" 9-step checklist
- `editors/vscode/AGENTS.md` — vscode extension architecture + command table
- `examples/playground/AGENTS.md` — POC authoring conventions
