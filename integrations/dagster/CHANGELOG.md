...
- **`execution_mode="pipes"` now actually delivers its events for two engine-side bugs (#2163, #2159); no source change here for either one.** Both engine bugs meant every Pipes-launched run either did nothing over the wire or crashed decoding the first message — see the engine CHANGELOG for the full detail. Practically: before upgrading the engine, `run_pipes()` and `RockyComponent(execution_mode="pipes")` looked idle or failed for reasons that had nothing to do with your `rocky.toml`; a plain `run_pipes()` call on an asset with no declared check specs went silently green with nothing reported, and a `RockyComponent` in pipes mode failed its step on every run because none of its declared checks ever got an answer. After upgrading the engine, real materializations and check results arrive, and a run failure you were not previously seeing may now surface. This package's own test suite gained two tests in `test_pipes_mode.py` that drive Dagster's real message-decoding path (not a mock) against a message stream captured from a real engine launch, to catch a regression on either side of that wire format again. (engine #2163, #2159)
