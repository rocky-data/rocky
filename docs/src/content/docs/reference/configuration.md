...
| `contain_failures` | bool | `false` | `false` stops the run at the model that fails **while it runs**: no later layer starts, though models already running beside it finish. A model that parses but fails **type-checking** does not stop the run — it is excluded and the others build. A project that does not compile at all builds nothing. See [Failure containment](/advanced/failure-modes/#failure-containment-across-the-model-graph). `true` withholds the failed model and everything downstream of it, and lets unrelated subtrees finish. It reports `PartialFailure` with a manifest naming what failed and its blast radius. |
