...
dbt Core 1.x is a templating engine; it can't catch any of these at compile time. SQLMesh moved correctness to the planner. **Rocky moved it to the compiler.** The disasters above become `E013` diagnostics, blocked PRs, `rocky lineage-diff` comments, and content-addressed run records.
