...
Be precise about what "acting as your user" means, because it is a property of how you deploy, not of Rocky. `.rocky/fulfillment/<product>/outbox/` is the one part of that directory the worker is meant to write: elicitation writes `candidate_spec.toml` there directly, and drafting's `draft_model` tool mirrors its draft there. The loop reads the outbox only after the worker exits and its process group is killed.

The claims above hold only while these stay closed to the same principal that runs the worker: `.rocky/plans/` (review markers — an unsigned marker with a matching plan id is accepted as approval), `.rocky/fulfillment/<product>/` except `outbox/` (task transcripts and the committed lowering manifest), the loop's journal (kept in the state store, not in that directory), and `products/` (approved specs). A worker that can write `models/` and the outbox, but not those, is the shape the gates are built for. A worker that can also write those is the hostile-local-process case, and nothing here defends against it. If your worker and your runner share a user, treat that as the concession it is.
