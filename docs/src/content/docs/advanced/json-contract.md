...
3. That exports the JSON schemas to `schemas/`, regenerates Pydantic v2 models in `integrations/dagster/src/dagster_rocky/types_generated/`, and TypeScript interfaces in `editors/vscode/src/types/generated/`.
...
Every JSON-emitting command is backed by a typed output schema in `schemas/`. See the [JSON Output](/reference/json-output/) reference for the per-command payload shapes.
