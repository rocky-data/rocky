...
**Rocky moves correctness into the compiler.** Each disaster above becomes something you catch before a row is written, invisible to the warehouse and out of reach of the templating layer above it. Rocky is the typed graph in between.
...
<CardGrid>
  ...
  <Card title="Branches + content-addressed run records" icon="random">
    Named branches as isolated schemas. `rocky replay <run_id>` inspects and verifies a run against its content-addressed record: per-model SQL hashes, row counts, and bytes. (Local re-execution ships today via `rocky replay --execute --verify`; warehouse re-execution is on the roadmap.)
  </Card>
  ...
  <Card title="Declarative tests and keys, not templated SQL" icon="list-format">
    Tests are declarative TOML and Rocky generates the assertion SQL for your target dialect. Define an assertion once in `models/test_definitions.toml` and apply it by name with `[[use_test]]`. Fixture-driven `[[test]]` blocks seed mock inputs, run the model SQL, and compare the rows on `rocky test` via DuckDB, no warehouse required. A `[[surrogate_key]]` block injects a deterministic hash into your SELECT whose value matches `dbt_utils.generate_surrogate_key` on the same warehouse.
  </Card>
  <Card title="A tested exit door, not a one-way door" icon="cloud-download">
    `rocky emit-sql` renders the compiled SQL for your transformation models with no warehouse connection. The output is plain runnable SQL with no Rocky-specific syntax, so adopting Rocky never locks you in.
  </Card>
</CardGrid>
...
`rocky run` writes a single `RunRecord` to the state store. Three read commands project that record from different angles: `rocky trace` for causality and concurrency, `rocky cost` for per-model warehouse spend, and `rocky replay` for the byte-for-byte reproduction. Walk through it end-to-end in [POC #17 (trace + cost + replay against the same run_id)](https://github.com/rocky-data/rocky/tree/main/examples/playground/pocs/06-developer-experience/17-trace-replay-cost-combo).
