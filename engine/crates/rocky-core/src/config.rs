...
///
/// `cas` satisfies that requirement today for the end-of-run upload and for the
/// `rocky policy` freeze/unfreeze ledger write (whose markers are create-once
/// and must never be replayed, so they stay outside the retry). It does not yet
/// for the ledger-seam writers reached through
/// `RemoteStateSession::upload_only_fail_closed` (`rocky gc`, `rocky apply`),
/// which still upload the shared object unconditionally on every backend — so
/// `cas` does not yet make a deployment fully safe against lost updates. Issue
