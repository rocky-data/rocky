 ...
 *
 * `cas` satisfies that requirement today for the end-of-run upload and for the `rocky policy` freeze/unfreeze ledger write (whose markers are create-once and must never be replayed, so they stay outside the retry). It does not yet for the ledger-seam writers reached through `RemoteStateSession::upload_only_fail_closed` (`rocky gc`, `rocky apply`), which still upload the shared object unconditionally on every backend — so `cas` does not yet make a deployment fully safe against lost updates. Issue #1228 tracks closing that half; `rocky doctor --check state_concurrency` reports the residual exposure in the meantime.
 */
...
export interface RockyConfig {
  ...
  /**
   * Agent-authority policy plane. Declares, per `(principal, capability, scope)`, whether an action is allowed, requires human review, or is denied; enforced at `apply`, `promote`, and the MCP write tools, with every decision recorded to the ledger. Absent `[policy]` block ⇒ no rules and no enforcement: the `apply` / `promote` / MCP-write gate returns `NotConfigured` and allows the action regardless of principal, so an absent block leaves behavior unchanged (it does NOT gate agents to review). One surface diverges: `rocky policy check` *predicts* against [`PolicyConfig::default_posture`] (agents on mutating actions → `require_review`, humans never gated), so with no `[policy]` block its prediction is stricter than what enforcement actually does. See [`PolicyConfig`] and [`crate::policy`] for the evaluator.
   */
 ...
 *
 * Eviction is **ledger-only**: an approved apply writes the durable tombstone and retires the artifact's ledger row (the eviction of record; the recipe itself stays in the provenance the tombstone references). Physical byte-deletion is not performed — reclaiming bytes safely requires a protocol-aware VACUUM (retention windows + TOCTOU-safe deletion), which is future work.
 *
 * `rocky restore` attempts to rebuild an evicted artifact only when the recorded recipe is non-partitioned, content-addressed, and reads no recorded upstreams. That shape is necessary, not sufficient: a supported recipe can still refuse on a missing tombstone/provenance binding, canonical IR that will not deserialize, unreachable object store or table state, a re-derivation whose blake3 differs from the tombstoned hash, a path outside the storage prefix, or a lost race on ledger reinstatement. A recipe with ANY recorded upstream cannot be restored today (multi-input DAG re-derivation is a later phase). Re-running the pipeline is not an equivalent: it recomputes the model from *current* upstreams and need not reproduce the evicted bytes, so if those upstreams have moved on the exact bytes are unrecoverable until multi-input restore lands. What is durably retained is the eviction record plus the referenced provenance, not a guarantee that the exact bytes remain recoverable.
 */
