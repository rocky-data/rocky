...
- **required**: these columns must exist with the declared types. The compiler fails if one is missing or has the wrong type.
...
- **Type inference**: it resolves column types through the chain. `amount` in `raw_orders` flows through `sum(amount)` in `customer_orders` into `total_revenue / order_count` in `revenue_summary`.
- **Contract validation**: it checks that `revenue_summary` outputs `customer_id` (Int64), `total_revenue` (Decimal), and `order_count` (Int64), as the contract requires.
