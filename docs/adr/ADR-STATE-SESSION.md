...
- **RD-003 (Critical, ACTIVE) — bypass.** Three entry points open a `StateStore` and mutate state-backed data with **no** remote download/upload:
  ...
  - `rocky load` / the pipeline `Load` arm — `run_load` opens `<config_dir>/.rocky_state` at `load.rs:104` and writes `loaded_files` (the replicated cross-pod load ledger; nothing reads it for load-skip decisions today) with no sync. Reached both standalone and via `run.rs:1596` (`PipelineConfig::Load` → `run_load` call at `run.rs:1600`).
