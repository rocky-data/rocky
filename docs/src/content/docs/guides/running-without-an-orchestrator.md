...
- **Shutdown drains.** On `SIGTERM` or `Ctrl-C` the server stops starting new work, gives a run already in flight up to `--drain-timeout-seconds` to finish on its own, and then terminates it — draining in-flight HTTP requests in the same window before the process exits.
