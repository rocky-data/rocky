...
//! ```
//!
//! The emission is "v0" by design: macros, dbt_packages, generic tests, and
//! singular tests are skipped and listed under "Not Translated" in
...
pub struct EmissionResult {
    ...
    /// Models whose `materialized` fell through to the v0 catch-all
...
fn render_model_sidecar(config: &ModelConfig) -> String {
    ...
    match &config.strategy {
        ...
        // The remaining variants don't arise from the v0 dbt importer
...
fn write_migration_notes(path: &Path, ctx: &MigrationContext<'_>) -> Result<(), String> {
    ...
    out.push_str("Generated by `rocky import-dbt`. This file summarises the v0 translation\n");
    out.push_str("of your dbt project into a runnable Rocky repo. Items under \"Not\n");
    out.push_str("Translated\" need a manual pass before `rocky run` will reproduce the dbt\n");
    ...
    out.push_str(&format!(
        "- dbt macros detected (NOT translated in v0): {}\n",
        ctx.macros_detected
    ));
    out.push('\n');
...
mod tests {
    ...
    fn view_models_get_rewritten_to_ephemeral() {
        ...
        // emit re-applies the v0 mapping (`view → ephemeral`) at write time.
        let imported = vec![make_model(
            "v_users",
            StrategyConfig::FullRefresh,
            "SELECT 1",
