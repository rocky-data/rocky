...
Use rust-analyzer's SSR for semantic code transformations in Rust projects. SSR matches code by AST structure and semantic meaning, not text. In Rocky this is especially useful for cross-crate refactors against the multi-crate workspace — string `grep` regularly over-matches.
...
Always dry-run with `parseOnly: true` or preview in the editor before applying workspace-wide — the multi-crate workspace makes a botched refactor expensive to back out.
