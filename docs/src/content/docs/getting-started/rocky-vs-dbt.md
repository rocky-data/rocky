...
Rocky reads the SQL itself. No Jinja: models are plain SQL, and the compiler parses each one, infers the columns and types it produces, and tracks how columns flow into the next model across the whole DAG. Statically detectable mistakes become compile errors before any model is materialized:
