...
- **Corrected `rocky gc` / `rocky restore` documentation that overstated recoverability.** The gc plan notes, apply notes, `[gc]` config reference, CLI help, JSON output schema, docs, and README told users an evicted artifact was "always restorable from the recorded recipe" and that `derivable` meant "provably rebuildable". Neither holds for a multi-input recipe: gc's eligibility check admits a `strong` closure over several content-hashed upstreams, but `rocky restore` refuses any recipe with recorded upstreams, so gc can evict artifacts restore cannot rebuild. `derivable` is now described as an eligibility verdict — a recipe was recorded and bound to the artifact's bytes — rather than a rebuild guarantee, and restore's supported recipe shape is stated as necessary rather than sufficient (a supported recipe can still refuse on a missing provenance binding, undeserializable canonical IR, unreachable storage, a hash that no longer reproduces, a path outside the storage prefix, or a lost reinstatement race). The eligibility-check enumeration also said "all five" while `build_candidate` applies six — the exact-output binding check was omitted. Documentation only; no behavior change, and the gc/restore asymmetry is pinned by a characterization test. (#1201)
