...
- **Group checks**: `cross_source_overlap` is evaluated once per `(source_type, table)` sibling group and reported on the sibling the engine copied first — not knowable at spec-build time. Its specs carry `GROUP_CHECK_METADATA_KEY`, and `_emit_placeholder_checks` gives the siblings without this run's verdict an explicit `passed=False` / `WARN` "not evaluated on this asset" result rather than a passing placeholder (#1669).
