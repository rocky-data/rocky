...
Source metadata  ◀── Fivetran REST API / DuckDB info_schema / manual config
    ...
    ▼
rocky-compiler  →  diagnostics (E001–E037 / W001–W031)
...
The 27 crates under `engine/crates/`, the two binary crates that ship as executables, and the browser UI:
...
│   ├── rocky-cli-compiletest/   # Compile-fail guard: rocky-cli's plan writers stay crate-private
...
│   ├── rocky-server/        # HTTP server support (auth, jobs, UI guard) + LSP server
│   ├── rocky-cache/         # Three-tier caching
│   ├── rocky-ai/            # AI intent layer
│   ├── rocky-fulfill/       # Fulfillment loop behind `rocky fulfill` (experimental)
...
│   └── rocky-cli/           # CLI framework, `/api/v1` routes, Dagster Pipes
├── rocky/                   # Binary crate (the `rocky` CLI)
├── rocky-lsp/               # Binary crate (the `rocky lsp` language server)
└── ui/                      # Browser UI (React), embedded by `rocky serve --ui`
```
...
Support for the HTTP server, and the Language Server Protocol (LSP) server.

- **HTTP support** (via `axum`): bearer-token auth and token scope, the async job model, webhook ingress, and the browser UI's `Host`/`Origin` guard and headers. The `/api/v1` routes themselves live in `rocky-cli` (`api.rs`), and so do the embedded UI files.
...
- **View**: a view object, so downstream consumers read it without copied data.
