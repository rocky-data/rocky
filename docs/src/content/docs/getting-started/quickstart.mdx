...
The [Playground](/guides/playground/) runs a local DuckDB transformation project with nothing to configure. It can compile, review emitted SQL, run models, and preview rows. `rocky plan` currently supports replication pipelines only. Use the Playground's guarded local run instead.
:::
...
Steps 1 to 4 below are the shared setup. Steps 5 and 6 are the PR-gated
replication path. Step 7 inspects what the run stored. For a local
transformation workflow, use the [Playground](/guides/playground/).
...
`rocky run` does the work of steps 5 and 6 in one command. Use it for replication iteration and for automation.
