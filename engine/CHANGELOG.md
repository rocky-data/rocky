...
- **`dagster-rocky`: detect `quota-exceeded` `failure_kind` on `RunResult.errors` and raise a retriable `dg.Failure`.** When the engine surfaces a quota-exceeded TableError (post-FR: sustained 429s tripping the shared breaker; pre-FR: exhausted retry budget), `_run_filters` accumulates the partial results, yields `MaterializeResult` / `AssetCheckResult` events for the succeeded filters via the existing `_emit_results` path, then raises `dg.Failure(allow_retries=True, metadata={"retry_after_seconds": MetadataValue.int(300), "failure_kind": "quota-exceeded"})`. The 300s default mirrors `CircuitConfig::default().cooldown`; per-error cooldown threading through `TableErrorOutput` is a follow-up.
