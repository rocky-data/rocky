...
///
/// **Scope:** `docs/adr/ADR-CONCURRENCY.md` governs. It *requires* every write
/// of the shared state object to compare-and-swap, split by writer class (runs
/// refuse on conflict; single-record ledger seams retry). That requirement
/// covers the shared object only: the sibling objects written under their own
/// keys — freeze markers, idempotency records, the doctor read/write probe —
/// are create-once or self-keyed, cannot lose an update, and stay outside
/// compare-and-swap by design rather than as a gap.
///
/// `cas` satisfies that requirement today for the end-of-run upload, for the
/// `rocky policy` freeze/unfreeze ledger write (whose markers are create-once
/// and must never be replayed, so they stay outside the retry), and — since
/// #1372 — for `rocky gc`. It does not yet for `rocky restore`, which uploads
/// the shared object unconditionally on every remote backend, nor for `rocky
/// apply`'s verify-after custody rows, which take that path only when
/// `verify_after` is configured. So `cas` does not yet make a deployment fully
/// safe against lost updates. Issue
/// #1228 tracks closing that half; `rocky doctor --check state_concurrency`
/// reports the residual exposure in the meantime.
//
...
pub struct CrossSourceOverlapConfig {
    ...
    /// refuses a statement terminator and any construct the target dialects lex
    /// differently, but does not bound the expression otherwise.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl CrossSourceOverlapConfig {
    ...
    /// through, and `generate_cross_source_overlap_sql` refuses a statement
    /// terminator or an ambiguously-lexed construct in it.
...
/// Check names the engine or the Dagster integration emits itself, which a
/// user assertion or custom check may not take.
///
...
/// error. Reserving these names is cheaper than detecting the collision
/// after it has eaten a result.
///
...
///
/// Keep in step with the Dagster side; each entry names its source:
///   - `contracts.py`     CONTRACT_REQUIRED_COLUMNS_CHECK and the two beside it
///   - `component.py`     DEFAULT_CHECK_NAMES
///   - `freshness.py`     FRESHNESS_CHECK_NAME
///   - `observability.py` ANOMALY_CHECK_NAME, COMPLIANCE_CHECK_NAME
///
/// This covers the FIXED names only. Names that are synthesized from user
/// input rather than written out — `null_rate:{column}`, an unnamed
/// assertion's generated name, cross-source names — can still collide with
/// each other. That needs a resolved per-table name set, tracked separately.
const RESERVED_CHECK_NAMES: &[&str] = &[
    // engine
    "quarantine:compile",
    // dagster contract checks
    "contract_required_columns",
    "contract_protected_columns",
    "contract_column_constraints",
    // dagster default checks
    "row_count",
    "column_match",
    "freshness",
    "row_count_anomaly",
...
"#
    ...
    /// Every name Dagster owns is refused, not just the engine's own.
    ///
    /// The first version reserved `quarantine:compile` alone. Dagster also
    /// pre-declares contract, default and compliance check specs, and its
    /// first-wins dedup drops the LATER spec with only a log line — so a user
    /// check colliding with one of those vanishes rather than erroring.
    ///
    /// `contract.required-columns` is the shape that motivated this: it is a
    /// plausible thing to call a check, and it sanitizes onto Dagster's
    /// `contract_required_columns`.
    #[test]
    fn a_name_dagster_owns_is_refused_on_an_assertion() {
        for spelling in [
            "contract.required-columns",
            "contract_protected_columns",
            "contract:column:constraints",
            "row_count",
            "column_match",
            "freshness",
            "row_count_anomaly",
...
"#
            ...
            assert!(
                ...
                "{spelling} collides with a dagster-owned check name and must be refused: \
                 {errors:?}"
            );
        }
    }
